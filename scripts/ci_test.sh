...
CURRENT_PATH=$(pwd)
TRAVIS_PATH='/home/travis/build/khulnasoft-lab/solscan'

# test_solscan file.sol detectors
test_solscan(){

    expected="$DIR/../tests/expected_json/$(basename "$1" .sol).$2.json"

    # run solscan detector on input file and save output as json
    if ! solscan "$1" --solc-disable-warnings --detect "$2" --json "$DIR/tmp-test.json";
    then
        echo "Solscan crashed"
        exit 255
    fi
    ...
    # run solscan detector on input file and save output as json
    if ! solscan "$1" --solc-disable-warnings --detect "$2" --legacy-ast --json "$DIR/tmp-test.json";
    then
        echo "Solscan crashed"
        exit 255
    fi
...
generate_expected_json(){
    ...
    # run solscan detector on input file and save output as json
    solscan "$1" --solc-disable-warnings --detect "$2" --json "$output_filename" > "$output_filename_txt" 2>&1
