...
from solscan.analyses.data_dependency.data_dependency import (
    is_dependent,
    is_tainted,
)
...
contracts = solscan.get_contract_from_name("Simple")
...
contracts = solscan.get_contract_from_name("Reference")
assert len(contracts) == 1
contract = contracts[0]
destination = contract.get_state_variable_from_name("destination")
assert destination
...
contracts = solscan.get_contract_from_name("SolidityVar")
assert len(contracts) == 1
contract = contracts[0]
...
contracts = solscan.get_contract_from_name("Intermediate")
assert len(contracts) == 1
contract = contracts[0]
destination = contract.get_state_variable_from_name("destination")
assert destination
...
contracts = solscan.get_contract_from_name("Base")
assert len(contracts) == 1
contract = contracts[0]
contract_derived = solscan.get_contract_from_name("Derived")[0]
destination = contract.get_state_variable_from_name("destination")
source = contract.get_state_variable_from_name("source")
assert destination
assert source
...
contracts = solscan.get_contract_from_name("PropagateThroughArguments")
assert len(contracts) == 1
contract = contracts[0]
