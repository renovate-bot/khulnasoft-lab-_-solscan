...
def _output_result_to_sarif(
    ...
    if not detector["elements"]:
        ...
        logger.info(yellow("This will be supported in a future Solscan release"))
        return
        ...
        "runs": [
            {
                "tool": {
                    "driver": {
                        "name": "Solscan",
                        "informationUri": "https://github.com/khulnasoft-lab/solscan",
...
def output_to_zip(filename: str, error: Optional[str], results: Dict, zip_type: str = "lzma"):
    ...
    The file in the zip is named solscan_results.json
    ...
    if os.path.isfile(filename):
        ...
        with ZipFile(
            ...
            file_desc.writestr("solscan_results.json", json.dumps(json_result).encode("utf8"))
...
def _convert_to_description(d: str) -> str:
    ...
    if not isinstance(d, SourceMapping):
        raise SolscanError(f"{d} does not inherit from SourceMapping, conversion impossible")
    ...
    raise SolscanError(f"{type(d)} cannot be converted (no name, or canonical_name")
...
class Output:
    ...
    def add(self, add: SupportedOutput, additional_fields: Optional[Dict] = None) -> None:
        ...
        if isinstance(add, Variable):
            ...
            raise SolscanError(f"Impossible to add {type(add)} to the json")
