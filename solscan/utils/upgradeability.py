...
from solscan.analyses.data_dependency.data_dependency import get_dependencies
...
from solscan.tools.read_storage.read_storage import SlotInfo, SolscanReadStorage
...
def compare(
    ...
    for function in v2.functions:
        if (
            ...
            or function.name.startswith("solscan")
        ):
            continue
        modified_calls = [
...
def get_proxy_implementation_slot(proxy: Contract) -> Optional[SlotInfo]:
    ...
    if isinstance(delegate, StateVariable):
        if not delegate.is_constant and not delegate.is_immutable:
            srs = SolscanReadStorage([proxy], 20)
        ...
        if dest.isnumeric():
            ...
            return next(
                (
                    ...
                    if SolscanReadStorage.get_variable_info(contract, v)[0] == slot_idx
                ),
                None,
            )
