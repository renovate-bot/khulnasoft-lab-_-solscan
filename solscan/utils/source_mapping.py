...
from solscan.core.declarations import Contract, Function, Enum, Event, Import, Pragma, Structure
from solscan.core.solidity_types.type import Type
from solscan.core.source_mapping.source_mapping import Source, SourceMapping
...
def get_definition(target: SourceMapping, crytic_compile: CryticCompile) -> Source:
    if isinstance(target, (Contract, Function, Enum, Event, Structure, Variable)):
        ...
        raise SolscanError("get_definition_generic not implemented for types")
    else:
        raise SolscanError(f"get_definition_generic not implemented for {type(target)}")
