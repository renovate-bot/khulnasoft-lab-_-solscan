...
from solscan.utils.tests_pattern import is_test_file
...
def compute_loc_metrics(solscan: Solscan) -> LoC:
    """Used to compute the lines of code metrics for a Solscan object.

    Args:
        solscan: A Solscan object
    Returns:
        A LoC object
    """
    ...
    for filename, source_code in solscan.source_code.items():
        current_lines = source_code.splitlines()
        is_dep = False
        if solscan.crytic_compile:
            is_dep = solscan.crytic_compile.is_dependency(filename)
