...
from solscan.tools.properties.addresses.address import Addresses
from solscan.tools.properties.platforms.echidna import generate_echidna_config
from solscan.tools.properties.properties.ercs.erc20.properties.burn import (
    ERC20_NotBurnable,
    ERC20_Burnable,
)
from solscan.tools.properties.properties.ercs.erc20.properties.initialization import ERC20_CONFIG
from solscan.tools.properties.properties.ercs.erc20.properties.mint import ERC20_NotMintable
from solscan.tools.properties.properties.ercs.erc20.properties.mint_and_burn import (
    ERC20_NotMintableNotBurnable,
)
from solscan.tools.properties.properties.ercs.erc20.properties.transfer import (
    ERC20_Transferable,
    ERC20_Pausable,
)
from solscan.tools.properties.properties.ercs.erc20.unit_tests.truffle import generate_truffle_test
from solscan.tools.properties.properties.properties import (
    property_to_solidity,
    Property,
)
from solscan.tools.properties.solidity.generate_properties import (
    generate_solidity_properties,
    generate_test_contract,
    generate_solidity_interface,
)
from solscan.utils.colors import red, green
...
def generate_erc20(
    ...
    if contract.compilation_unit.core.crytic_compile.type not in [
        ...
        logging.error(
            f"{contract.compilation_unit.core.crytic_compile.type} not yet supported by solscan-prop"
        )
        return
...
def _check_compatibility(contract):
    ...
    if not contract.is_erc20():
        errors = f"{contract} is not ERC20 compliant. Consider checking the contract with solscan-check-erc"
        return errors

    transfer = contract.get_function_from_signature("transfer(address,uint256)")

    if transfer.visibility != "public":
        errors = f"solscan-prop requires {transfer.canonical_name} to be public. Please change the visibility"
    ...
    if transfer_from.visibility != "public":
        if errors:
            errors += "\n"
        errors += f"solscan-prop requires {transfer_from.canonical_name} to be public. Please change the visibility"

    approve = contract.get_function_from_signature("approve(address,uint256)")
    if approve.visibility != "public":
        if errors:
            errors += "\n"
        errors += f"solscan-prop requires {approve.canonical_name} to be public. Please change the visibility"
