...
from solscan.tools.read_storage.read_storage import SolscanReadStorage, RpcInfo
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        ...
        usage=(
            ...
            + "\tsolscan-read-storage $TARGET address --variable-name $NAME\n"
            + "To retrieve a contract's storage layout:\n"
            + "\tsolscan-read-storage $TARGET address --contract-name $NAME --json storage_layout.json\n"
            + "To retrieve a contract's storage layout and values:\n"
            + "\tsolscan-read-storage $TARGET address --contract-name $NAME --json storage_layout.json --value\n"
...
def main() -> None:
    ...
    if len(args.contract_source) == 2:
        ...
        solscan = Solscan(source_code, **vars(args))
    else:
        # Source code is published and retrieved via etherscan
        target = args.contract_source[0]
        solscan = Solscan(target, **vars(args))

    if args.contract_name:
        contracts = solscan.get_contract_from_name(args.contract_name)
    else:
        contracts = solscan.contracts
    ...
    srs = SolscanReadStorage(contracts, args.max_depth, rpc_info)
