...
def check_solscan_path(**_kwargs) -> None:
    binary_here, binary_path, possible_paths = check_path_config("solscan")
    show_paths = False

    if binary_path:
        print(green(f"`solscan` found in PATH at `{binary_path}`."))
        if binary_here:
            print(green("Its location matches this solscan-doctor installation."))
        else:
            print(
                yellow(
                    "This path does not correspond to this solscan-doctor installation.\n"
                    + "Double-check the order of directories in PATH if you have several Solscan installations."
                )
            )
            show_paths = True
    else:
        print(red("`solscan` was not found in PATH."))
        show_paths = True
