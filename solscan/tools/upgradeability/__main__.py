...
from solscan.utils.output import output_to_json
from solscan.tools.upgradeability.checks import all_checks
from solscan.tools.upgradeability.checks.abstract_checks import (
    AbstractCheck,
    CheckClassification,
)
from solscan.tools.upgradeability.utils.command_line import (
...
logging.basicConfig()
logger: logging.Logger = logging.getLogger("Solscan")
logger.setLevel(logging.INFO)


def parse_args(check_classes: List[Type[AbstractCheck]]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Solscan Upgradeability Checks. For usage information see https://github.com/khulnasoft-lab/solscan/wiki/Upgradeability-Checks.",
        usage="solscan-check-upgradeability contract.sol ContractName",
    )
...
def main() -> None:
    ...
    try:
        variable1 = Solscan(v1_filename, **vars(args))
        ...
        if args.proxy_name:
            if args.proxy_filename:
                proxy = Solscan(args.proxy_filename, **vars(args))
            else:
                proxy = variable1
        ...
        if args.new_contract_name:
            if args.new_contract_filename:
                variable2 = Solscan(args.new_contract_filename, **vars(args))
            else:
                variable2 = variable1
        ...
        logger.error(str(solscan_exception))
        if args.json:
            output_to_json(args.json, str(solscan_exception), json_results)
        return
