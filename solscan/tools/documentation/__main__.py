...
from solscan.formatters.utils.patches import create_patch, apply_patch, create_diff
...
def parse_args() -> argparse.Namespace:
    ...
    """
    parser = argparse.ArgumentParser(description="Demo", usage="solscan-documentation filename")
...
def _post_processesing(
    ...
    # In this case, we allow solscan-documentation to force the */
    if answer.count("*/") != 1:
...
def _handle_function(
    ...
    while tentative < retry:
        ...
        answer = openai.Completion.create(  # type: ignore
            prompt=prompt,
            model=solscan.codex_model,
            temperature=min(solscan.codex_temperature + tentative * 0.1, 1),
            max_tokens=solscan.codex_max_tokens,
        )
    ...
    for scope in compilation_unit.scopes.values():
        ...
        for function in functions_target:
            overwrite = _handle_function(
                function, overwrite, all_patches, logging_file, solscan, retry, force
            )
...
def main() -> None:
    ...
    solscan = Solscan(args.project, **vars(args))
