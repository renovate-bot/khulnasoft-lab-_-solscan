...
from solscan.core.declarations import Function, FunctionContract
...
def resolve_function(solscan: SolscanCore, contract_name: str, function_name: str) -> Function:
    """
    ...
    contracts = solscan.get_contract_from_name(contract_name)
...
def resolve_functions(
    solscan: Solscan, functions: List[Union[str, Tuple[str, str]]]
) -> List[Function]:
    """
    ...
    for item in functions:
        if isinstance(item, str):
            ...
            resolved.append(resolve_function(solscan, parts[0], parts[1]))
            ...
            resolved.append(resolve_function(solscan, item[0], item[1]))
        else:
            raise ResolveFunctionException(
...
def __find_target_paths(
    solscan: SolscanCore, target_function: Function, current_path: Optional[List[Function]] = None
) -> Set[Tuple[Function, ...]]:
    ...
    for contract in solscan.contracts:
        for function in contract.functions_and_modifiers_declared:
            ...
            if all_target_functions.intersection(called_functions):
                path_results = __find_target_paths(solscan, function, current_path.copy())
                if path_results:
...
def find_target_paths(
    solscan: SolscanCore, target_functions: List[Function]
) -> Set[Tuple[Function, ...]]:
    """
    ...
    for target_function in target_functions:
        results = results.union(__find_target_paths(solscan, target_function))
