...
from solscan.core.declarations import FunctionContract
from solscan.core.variables.state_variable import StateVariable
from solscan.utils import output
from solscan.utils.colors import yellow, green, red

logging.basicConfig(level=logging.WARNING)
logger = logging.getLogger("Solscan.kspec")
...
def _get_solscan_functions(
    ...
    all_functions_declared += list(
        {s for s in solscan.state_variables if s.visibility in ["public", "external"]}
    )
    solscan_functions = {
...
def _run_coverage_analysis(
    args: Namespace, solscan: SolscanCompilationUnit, kspec_functions: Set[Tuple[str, str]]
) -> None:
    # Collect all solscan functions
    solscan_functions = _get_solscan_functions(solscan)

    # Determine which klab specs were not resolved.
    solscan_functions_set = set(solscan_functions)
    kspec_functions_resolved = kspec_functions & solscan_functions_set
    ...
    for solscan_func_desc in sorted(solscan_functions_set):
        solscan_func = solscan_functions[solscan_func_desc]

        if solscan_func_desc in kspec_functions:
            kspec_present.append(solscan_func)
        else:
            kspec_missing.append(solscan_func)
...
def run_analysis(args: Namespace, solscan: SolscanCompilationUnit, kspec_arg: str) -> None:
    ...
    _run_coverage_analysis(args, solscan, kspec_functions)
