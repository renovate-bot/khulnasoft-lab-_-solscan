...
from solscan.detectors.variables.unused_state_variables import UnusedStateVars
from solscan.detectors.attributes.incorrect_solc import IncorrectSolc
from solscan.detectors.attributes.constant_pragma import ConstantPragma
from solscan.detectors.naming_convention.naming_convention import NamingConvention
from solscan.detectors.functions.external_function import ExternalFunction
from solscan.detectors.variables.could_be_constant import CouldBeConstant
from solscan.detectors.variables.could_be_immutable import CouldBeImmutable
from solscan.detectors.attributes.const_functions_asm import ConstantFunctionsAsm
from solscan.detectors.attributes.const_functions_state import ConstantFunctionsState
from solscan.utils.colors import yellow
...
def solscan_format(solscan: Solscan, **kwargs: Dict) -> None:  # pylint: disable=too-many-locals
    """'
    Keyword Args:
    ...
    solscan.generate_patches = True

    detector_results = solscan.run_detectors()
    ...
    logger.info(yellow("solscan-format is in beta, carefully review each patch before merging it."))
...
def print_patches(number_of_solscan_results: int, patches: Dict) -> None:
    logger.info("Number of Solscan results: " + str(number_of_solscan_results))
    number_of_patches = 0
    for file in patches:
...
def print_patches_json(number_of_solscan_results: int, patches: Dict) -> None:
    print("{", end="")
    print('"Number of Solscan results":' + '"' + str(number_of_solscan_results) + '",')
