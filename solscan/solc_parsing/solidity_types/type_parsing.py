...
from solscan.core.declarations.custom_error_contract import CustomErrorContract
...
from solscan.core.solidity_types import TypeAlias, TypeAliasTopLevel, TypeAliasContract
from solscan.core.solidity_types.array_type import ArrayType
from solscan.core.solidity_types.elementary_type import (
    ElementaryType,
    ElementaryTypeName,
)
from solscan.core.solidity_types.function_type import FunctionType
from solscan.core.solidity_types.mapping_type import MappingType
...
from solscan.core.variables.function_type_variable import FunctionTypeVariable
from solscan.exceptions import SolscanError
from solscan.solc_parsing.exceptions import ParsingError
from solscan.solc_parsing.expressions.expression_parsing import CallerContextExpression

if TYPE_CHECKING:
    from solscan.core.declarations import Structure, Enum, Function
...
def _find_from_type_name(  # pylint: disable=too-many-locals,too-many-branches,too-many-statements,too-many-arguments
    ...
    if not var_type:
        ...
        # all_enums += contract.solscan.enums_top_level
    ...
    if not var_type:
        ...
        # all_structures += contract.solscan.structures_top_level
...
def _add_type_references(type_found: Type, src: str, sl: "SolscanCompilationUnit") -> None:
...
def parse_type(
    ...
    caller_context: Union[CallerContextExpression, "SolscanCompilationUnitSolc"],
) -> Type:
    """
    caller_context can be a SolscanCompilationUnitSolc because we recursively call the function
    and go up in the context's scope. If we are really lost we just go over the SolscanCompilationUnitSolc
    ...
    from solscan.solc_parsing.expressions.expression_parsing import parse_expression
    from solscan.solc_parsing.variables.function_type_variable import FunctionTypeVariableSolc
    ...
    if isinstance(caller_context, SolscanCompilationUnitSolc) or (
        isinstance(caller_context, FunctionSolc) and caller_context.contract_parser is None
    ):
        structures_direct_access: List["Structure"]
        if isinstance(caller_context, SolscanCompilationUnitSolc):
            ...
            next_context = caller_context.solscan_parser
    ...
    if t[key] == "IdentifierPath":
        ...
        raise SolscanError("Solidity 0.8 not supported with the legacy AST")
