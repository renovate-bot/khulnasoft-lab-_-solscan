...
from solscan.analyses.data_dependency.data_dependency import compute_dependency
from solscan.core.compilation_unit import SolscanCompilationUnit
from solscan.core.declarations import Contract
from solscan.core.declarations.custom_error_top_level import CustomErrorTopLevel
from solscan.core.declarations.enum_top_level import EnumTopLevel
from solscan.core.declarations.function_top_level import FunctionTopLevel
from solscan.core.declarations.import_directive import Import
from solscan.core.declarations.pragma_directive import Pragma
from solscan.core.declarations.structure_top_level import StructureTopLevel
from solscan.core.declarations.using_for_top_level import UsingForTopLevel
from solscan.core.scope.scope import FileScope
from solscan.core.solidity_types import ElementaryType, TypeAliasTopLevel
...
from solscan.solc_parsing.declarations.contract import ContractSolc
from solscan.solc_parsing.declarations.custom_error import CustomErrorSolc
from solscan.solc_parsing.declarations.function import FunctionSolc
from solscan.solc_parsing.declarations.structure_top_level import StructureTopLevelSolc
from solscan.solc_parsing.declarations.using_for_top_level import UsingForTopLevelSolc
from solscan.solc_parsing.exceptions import VariableNotFound
from solscan.solc_parsing.variables.top_level_variable import TopLevelVariableSolc

logging.basicConfig()
logger = logging.getLogger("SolscanSolcParsing")
logger.setLevel(logging.INFO)


class InheritanceResolutionError(SolscanException):
    pass
...
def _handle_import_aliases(
    ...
    for symbol_alias in symbol_aliases:
        if "foreign" in symbol_alias and "local" in symbol_alias:
            ...
            # see https://github.com/khulnasoft-lab/solscan/issues/1319
            elif symbol_alias["local"]:
                raise SolscanException(
                    "Cannot resolve local alias for import directive due to malformed AST. Please upgrade to solc 0.6.0 or higher."
                )


class SolscanCompilationUnitSolc(CallerContextExpression):
    # pylint: disable=no-self-use,too-many-instance-attributes
    def __init__(self, compilation_unit: SolscanCompilationUnit) -> None:
        super().__init__()
    ...
    def parse_top_level_from_loaded_json(self, data_loaded: Dict, filename: str) -> None:
        ...
        for top_level_data in data_loaded[self.get_children()]:
            if top_level_data[self.get_key()] == "ContractDefinition":
                ...
                raise SolscanException(f"Top level {top_level_data[self.get_key()]} not supported")
    ...
    def parse_contracts(self) -> None:  # pylint: disable=too-many-statements,too-many-branches
        ...
        for contract in self._underlying_contract_to_parser:
            if contract.name.startswith("SolscanInternalTopLevelContract"):
                raise SolscanException(
                    # region multi-line-string
                    """Your codebase has a contract named 'SolscanInternalTopLevelContract'.
Please rename it, this name is reserved for Solscan's internals"""
                ...
                if i in contract_parser.remapping:
                    ...
                    if target == contract_parser.underlying_contract:
                        raise InheritanceResolutionError(
                            "Could not resolve contract inheritance. This is likely caused by an import renaming that collides with existing names (see https://github.com/khulnasoft-lab/solscan/issues/1758)."
    ...
    def analyze_contracts(self) -> None:  # pylint: disable=too-many-statements,too-many-branches
        if not self._parsed:
            raise SolscanException("Parse the contract before running analyses")
    ...
    def _analyze_top_level_structures(self) -> None:
        try:
            ...
            raise SolscanException(f"Missing struct {e} during top level structure analyze") from e
    ...
    def _analyze_top_level_variables(self) -> None:
        try:
            ...
            raise SolscanException(f"Missing {e} during variable analyze") from e
    ...
    def _analyze_content_top_level_function(self) -> None:
        try:
            ...
            raise SolscanException(f"Missing {e} during top level function analyze") from e
    ...
    def _convert_to_slithir(self) -> None:
        ...
        for contract in self._compilation_unit.contracts:
            ...
            for func in contract.functions + contract.modifiers:
                try:
                    ...
                    logger.error(
                        f"\nFailed to generate IR for {contract.name}.{func.name}. Please open an issue https://github.com/khulnasoft-lab/solscan/issues.\n{contract.name}.{func.name} ({func.source_mapping}):\n "
                        f"{func_expressions}"
                    )
                    raise e
            try:
                ...
                logger.error(
                    f"\nFailed to convert IR to SSA for {contract.name} contract. Please open an issue https://github.com/khulnasoft-lab/solscan/issues.\n "
                )
                raise e
        ...
        for func in self._compilation_unit.functions_top_level:
            try:
                ...
                func_expressions = "\n".join([f"\t{ex}" for ex in func.expressions])
                logger.error(
                    f"\nFailed to generate IR for top level function {func.name}. Please open an issue https://github.com/khulnasoft-lab/solscan/issues.\n{func.name} ({func.source_mapping}):\n "
                    f"{func_expressions}"
                )
                raise e
            ...
            try:
                ...
                func_expressions = "\n".join([f"\t{ex}" for ex in func.expressions])
                logger.error(
                    f"\nFailed to convert IR to SSA for top level function {func.name}. Please open an issue https://github.com/khulnasoft-lab/solscan/issues.\n{func.name} ({func.source_mapping}):\n "
                    f"{func_expressions}"
                )
                raise e
