...
from solscan.core.cfg.node import NodeType, Node, link_nodes
...
from solscan.core.declarations.function import FunctionLanguage
...
from solscan.core.solidity_types import ElementaryType
...
from solscan.solc_parsing.yul.evm_functions import (
...
from solscan.solc_parsing.expressions.find_variable import find_top_level
from solscan.visitors.expression.find_calls import FindCalls
from solscan.visitors.expression.read_var import ReadVar
from solscan.visitors.expression.write_var import WriteVar
...
class YulFunction(YulScope):
    ...
    def new_node(self, node_type: NodeType, src: str) -> YulNode:
        if self._function:
            ...
            raise SolscanException("standalone yul objects are not supported yet")
...
def parse_yul_function_call(root: YulScope, node: YulNode, ast: Dict) -> Optional[Expression]:
    ...
    if not isinstance(ident, Identifier):
        raise SolscanException("expected identifier from parsing function name")
    ...
    raise SolscanException(f"unexpected function call target type {str(type(ident.value))}")
...
def _parse_yul_magic_suffixes(name: str, root: YulScope) -> Optional[Expression]:
    ...
    if name.endswith(".length"):
        ...
        # The code below is an hotfix to allow solscan to process length in yul
...
def parse_yul_identifier(root: YulScope, _node: YulNode, ast: Dict) -> Optional[Expression]:
    ...
    raise SolscanException(f"unresolved reference to identifier {name}")
...
def parse_yul_unsupported(_root: YulScope, _node: YulNode, ast: Dict) -> Optional[Expression]:
    raise SolscanException(f"no parser available for {ast['nodeType']} {json.dumps(ast, indent=2)}")
