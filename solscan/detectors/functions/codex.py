...
class Codex(AbstractDetector):
    ...
    WIKI = "https://github.com/khulnasoft-lab/solscan/wiki/Detector-Documentation#codex"
    ...
    def _run_codex(self, logging_file: str, prompt: str) -> str:
        ...
        if self.solscan.codex_log:
            codex.log_codex(logging_file, "Q: " + prompt)

        answer = ""
        res = {}

        if self.solscan.codex_organization:
            openai_module.organization = self.solscan.codex_organization

        try:
            res = openai_module.Completion.create(
                prompt=prompt,
                model=self.solscan.codex_model,
                temperature=self.solscan.codex_temperature,
                max_tokens=self.solscan.codex_max_tokens,
            )
    ...
    def _detect(self) -> List[Output]:
        ...
        if not self.solscan.codex_enabled:
            return []
        ...
        for contract in self.compilation_unit.contracts:
            if (
                self.solscan.codex_contracts != "all"
                and contract.name not in self.solscan.codex_contracts.split(",")
            ):
                continue
