...
from solscan.printers import all_printers
from solscan.printers.abstract_printer import AbstractPrinter
from solscan.solscan import Solscan
from solscan.utils import codex
from solscan.utils.output import (
    output_to_json,
    output_to_zip,
    output_to_sarif,
    ZIP_TYPES_ACCEPTED,
...
from solscan.utils.output_capture import StandardOutputCapture
from solscan.utils.colors import red, set_colorization_enabled
from solscan.utils.command_line import (
    FailOnLevel,
    output_detectors,
...
def process_single(
...
) -> Tuple[Solscan, List[Dict], List[Output], int]:
    """
    The core high-level code for running Solscan static analysis.
    ...
    if args.legacy_ast:
        ast = "--ast-json"
    solscan = Solscan(target, ast_format=ast, **vars(args))

    return _process(solscan, detector_classes, printer_classes)
...
def process_all(
    ...
    detector_classes: List[Type[AbstractDetector]],
    printer_classes: List[Type[AbstractPrinter]],
) -> Tuple[List[Solscan], List[Dict], List[Output], int]:
    ...
    for compilation in compilations:
        ...
        solscan_instances.append(solscan)
...
def _process(
    ...
    for detector_cls in detector_classes:
        solscan.register_detector(detector_cls)

    for printer_cls in printer_classes:
        solscan.register_printer(printer_cls)

    analyzed_contracts_count = len(solscan.contracts)

    results_detectors = []
    results_printers = []

    if not printer_classes:
        detector_resultss = solscan.run_detectors()
        ...
        printer_results = solscan.run_printers()
        printer_results = [x for x in printer_results if x]  # remove empty results
        results_printers.extend(printer_results)

    return solscan, results_detectors, results_printers, analyzed_contracts_count
...
def get_detectors_and_printers() -> Tuple[
    ...
    # Handle plugins!
    for entry_point in iter_entry_points(group="solscan_analyzer.plugin", name=None):
...
def parse_args(
    ...
    parser.add_argument(
        ...
        version=require("solscan-analyzer")[0].version,
        action="version",
    )
    ...
    group_misc.add_argument(
        "--triage-mode",
        help="Run triage mode (save results in solscan.db.json)",
        action="store_true",
        dest="triage_mode",
        default=False,
    )
...
def main_impl(
    ...
    """
    # Set logger of Solscan to info, to catch warnings related to the arg parsing
    ...
    try:
        ...
        if args.solc_ast or (filename.endswith(".json") and not is_supported(filename)):
            ...
            for filename in filenames:
                ...
                solscan_instances.append(solscan_instance)
        ...
        if outputting_json or outputting_zip or output_to_sarif:
            ...
            if "compilations" in args.json_types:
                compilation_results = []
                for solscan_instance in solscan_instances:
                    assert solscan_instance.crytic_compile
                    compilation_results.append(
                        generate_standard_export(solscan_instance.crytic_compile)
                    )
    ...
    except SolscanException as solscan_exception:
        output_error = str(solscan_exception)
        traceback.print_exc()
