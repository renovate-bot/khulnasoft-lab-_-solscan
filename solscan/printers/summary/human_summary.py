...
from solscan.core.declarations import SolidityFunction, Function
...
from solscan.printers.summary.loc import compute_loc_metrics
...
from solscan.utils.code_complexity import compute_cyclomatic_complexity
from solscan.utils.colors import green, red, yellow
from solscan.utils.myprettytable import MyPrettyTable
from solscan.utils.standard_libraries import is_standard_library
...
class PrinterHumanSummary(AbstractPrinter):
    ...
    WIKI = "https://github.com/khulnasoft-lab/solscan/wiki/Printer-documentation#human-summary"
    ...
    def _get_detectors_result(self) -> Tuple[List[Dict], int, int, int, int, int]:
        ...
        checks_optimization = self.solscan.detectors_optimization
        checks_informational = self.solscan.detectors_informational
        checks_low = self.solscan.detectors_low
        checks_medium = self.solscan.detectors_medium
        checks_high = self.solscan.detectors_high
    ...
    def _compilation_type(self):
        if self.solscan.crytic_compile is None:
            return "Compilation non standard\n"
        return f"Compiled with {str(self.solscan.crytic_compile.type)}\n"

    def _number_contracts(self) -> Tuple[int, int, int]:
        contracts = self.solscan.contracts
    ...
    def _get_features(self, contract):  # pylint: disable=too-many-branches
        ...
        for compilation_unit in self.solscan.compilation_units:
    ...
    def output(self, _filename):  # pylint: disable=too-many-locals,too-many-statements
        ...
        for contract in self.solscan.contracts_derived:
