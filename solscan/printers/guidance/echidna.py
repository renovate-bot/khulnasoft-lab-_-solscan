...
from solscan.analyses.data_dependency.data_dependency import is_dependent
from solscan.core.cfg.node import Node
from solscan.core.declarations import Enum, Function
from solscan.core.declarations.solidity_variables import (
    SolidityVariableComposed,
    SolidityFunction,
    SolidityVariable,
)
from solscan.core.expressions import NewContract
from solscan.core.solscan_core import SolscanCore
from solscan.core.solidity_types import TypeAlias
...
from solscan.slithir.operations.binary import Binary
from solscan.slithir.variables import Constant
from solscan.utils.output import Output
from solscan.visitors.expression.constants_folding import ConstantFolding, NotConstant
...
def _extract_payable(solscan: SolscanCore) -> Dict[str, List[str]]:
...
def _extract_solidity_variable_usage(
    solscan: SolscanCore, sol_var: SolidityVariable
) -> Dict[str, List[str]]:
    ret: Dict[str, List[str]] = {}
    for contract in solscan.contracts:
...
def _is_constant(f: Function) -> bool:  # pylint: disable=too-many-branches
    ...
    - If it contains assembly -> Return false (SolscanCore doesn't analyze asm)
...
def _extract_constant_functions(solscan: SolscanCore) -> Dict[str, List[str]]:
    ret: Dict[str, List[str]] = {}
    for contract in solscan.contracts:
...
def _extract_assert(solscan: SolscanCore) -> Dict[str, List[str]]:
    ret: Dict[str, List[str]] = {}
    for contract in solscan.contracts:
...
def _have_external_calls(solscan: SolscanCore) -> Dict[str, List[str]]:
    """
...
def _use_balance(solscan: SolscanCore) -> Dict[str, List[str]]:
    """
    Detect the functions with external calls
    :param solscan:
    :return:
...
def _with_fallback(solscan: SolscanCore) -> Set[str]:
    ret: Set[str] = set()
    for contract in solscan.contracts:
        for function in contract.functions_entry_points:
...
def _with_receive(solscan: SolscanCore) -> Set[str]:
    ret: Set[str] = set()
    for contract in solscan.contracts:
        for function in contract.functions_entry_points:
...
def _call_a_parameter(solscan: SolscanCore) -> Dict[str, List[Dict]]:
    """
    Detect the functions with external calls
    :param solscan:
    :return:
    ...
    for contract in solscan.contracts:  # pylint: disable=too-many-nested-blocks
        for function in contract.functions_entry_points:
            try:
                for ir in function.all_slithir_operations():
                    if isinstance(ir, HighLevelCall):
...
class Echidna(AbstractPrinter):
    ...
    WIKI = "https://github.com/khulnasoft-lab/solscan/wiki/Printer-documentation#echidna"
    ...
    def output(self, filename: str) -> Output:  # pylint: disable=too-many-locals
        ...
        payable = _extract_payable(self.solscan)
        timestamp = _extract_solidity_variable_usage(
            self.solscan, SolidityVariableComposed("block.timestamp")
        )
        block_number = _extract_solidity_variable_usage(
            self.solscan, SolidityVariableComposed("block.number")
        )
        msg_sender = _extract_solidity_variable_usage(
            self.solscan, SolidityVariableComposed("msg.sender")
        )
        msg_gas = _extract_solidity_variable_usage(
            self.solscan, SolidityVariableComposed("msg.gas")
        )
        assert_usage = _extract_assert(self.solscan)
        cst_functions = _extract_constant_functions(self.solscan)
        (cst_used, cst_used_in_binary) = _extract_constants(self.solscan)

        functions_relations = _extract_function_relations(self.solscan)
        ...
        external_calls = _have_external_calls(self.solscan)

        call_parameters = _call_a_parameter(self.solscan)

        use_balance = _use_balance(self.solscan)

        with_fallback = list(_with_fallback(self.solscan))

        with_receive = list(_with_receive(self.solscan))
        ...
        d = {
            ...
            "solc_versions": [unit.solc_version for unit in self.solscan.compilation_units],
