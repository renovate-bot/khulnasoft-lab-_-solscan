...
from solscan.utils.inheritance_analysis import (
...
class PrinterInheritanceGraph(AbstractPrinter):
    ...
    WIKI = "https://github.com/khulnasoft-lab/solscan/wiki/Printer-documentation#inheritance-graph"

    def __init__(self, solscan, logger):
        super().__init__(solscan, logger)

        inheritance = [x.inheritance for x in solscan.contracts]
        self.inheritance = {item for sublist in inheritance for item in sublist}

        self.overshadowing_state_variables = {}
        shadows = detect_state_variable_shadowing(solscan.contracts)
