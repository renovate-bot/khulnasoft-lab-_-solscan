...
from solscan.slithir.operations import (
    Send,
    Transfer,
    OperationWithLValue,
    HighLevelCall,
...
from solscan.core.declarations import Modifier, Event
from solscan.core.solidity_types import UserDefinedType, MappingType
from solscan.core.declarations import Enum, Contract, Structure, Function
from solscan.core.solidity_types.elementary_type import ElementaryTypeName
from solscan.core.variables.local_variable import LocalVariable
from solscan.formatters.exceptions import FormatError, FormatImpossible
from solscan.formatters.utils.patches import create_patch

logging.basicConfig(level=logging.INFO)
logger = logging.getLogger("Solscan.Format")
...
def _name_already_use(solscan: SolscanCompilationUnit, name: str) -> bool:
    # Do not convert to a name used somewhere else
    if not KEY in solscan.context:
        all_names: Set[str] = set()
        for contract in solscan.contracts_derived:
        ...
        solscan.context[KEY] = all_names
    return name in solscan.context[KEY]


def _convert_CapWords(original_name: str, solscan: SolscanCompilationUnit) -> str:
    ...
    if _name_already_use(solscan, name):
...
def _convert_mixedCase(
    original_name: Union[str, bytes], compilation_unit: SolscanCompilationUnit
) -> str:
...
def _convert_UPPER_CASE_WITH_UNDERSCORES(
    name: str, compilation_unit: SolscanCompilationUnit
) -> str:
...
CONVENTION_F_TYPE = Callable[[str, SolscanCompilationUnit], str]
...
def _get_from_contract(
    compilation_unit: SolscanCompilationUnit, element: Dict, name: str, getter: str
) -> TARGET_TYPE:
...
def _patch(
    compilation_unit: SolscanCompilationUnit, result: Dict, element: Dict, _target: str
) -> None:
    scope = compilation_unit.get_scope(element["source_mapping"]["filename_absolute"])
...
def _is_var_declaration(solscan: SolscanCompilationUnit, filename: str, start: int) -> bool:
    """
    ...
    """
    v = "var "
    return solscan.core.source_code[filename][start : start + len(v)] == v


def _explore_type(  # pylint: disable=too-many-arguments,too-many-locals,too-many-branches
    solscan: SolscanCompilationUnit,
    result: Dict,
    target: TARGET_TYPE,
    ...
    if isinstance(custom_type, UserDefinedType):
        ...
        if isinstance(custom_type.type, (Enum, Contract)):
            if custom_type.type == target:
                ...
                new_str = convert(old_str, solscan)

                loc_start = start
                if _is_var_declaration(solscan, filename_source_code, start):
            ...
            _explore_variables_declaration(
                solscan, list(custom_type.type.elems.values()), result, target, convert
            )
    ...
    if isinstance(custom_type, MappingType):
        ...
        if isinstance(custom_type.type_to, (UserDefinedType, MappingType)) or target in [
            ...
            full_txt_end = end
            full_txt = solscan.core.source_code[filename_source_code].encode("utf8")[
            ...
            if patch_comment and isinstance(variable, LocalVariable):
                if variable.source_mapping.lines:
                    ...
                    if variable in func.parameters:
                        ...
                        potential_comments_ = solscan.core.source_code[filename_source_code].encode(
                            "utf8"
                        )
...
def _explore_structures_declaration(
    ...
    for st in structures:
        ...
        _explore_variables_declaration(solscan, list(st.elems.values()), result, target, convert)
...
def _explore_events_declaration(
    ...
    for event in events:
        ...
        _explore_variables_declaration(solscan, event.elems, result, target, convert)
...
def _explore_functions(
    ...
    for function in functions:
        _explore_variables_declaration(solscan, function.variables, result, target, convert, True)
        _explore_irs(solscan, function.all_slithir_operations(), result, target, convert)
...
def _explore_contract(
    ...
    _explore_variables_declaration(solscan, contract.state_variables, result, target, convert)
    _explore_structures_declaration(solscan, contract.structures, result, target, convert)
    _explore_functions(solscan, contract.functions_and_modifiers, result, target, convert)
    _explore_enums(solscan, contract.enums, result, target, convert)
...
def _explore(
    compilation_unit: SolscanCompilationUnit,
    result: Dict,
    target: TARGET_TYPE,
    convert: CONVENTION_F_TYPE,
) -> None:
