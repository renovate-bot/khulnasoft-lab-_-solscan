...
class ElementaryType(Type):
    ...
    def size(self) -> Optional[int]:
        ...
        raise SolscanException(f"{t} does not have a size")
    ...
    def min(self) -> int:
        ...
        raise SolscanException(f"{self.name} does not have a min value")

    @property
    def max(self) -> int:
        if self.name in MaxValues:
            return MaxValues[self.name]
        raise SolscanException(f"{self.name} does not have a max value")
