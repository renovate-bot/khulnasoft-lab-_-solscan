...
from solscan.core.declarations import Contract, FunctionContract
from solscan.core.declarations.top_level import TopLevel
from solscan.core.source_mapping.source_mapping import SourceMapping, Source
from solscan.slithir.variables import Constant
from solscan.utils.colors import red
from solscan.utils.source_mapping import get_definition, get_references, get_implementation
...
class SolscanCore(Context):
    """
    Solscan static analyzer
    """
    ...
    def __init__(self) -> None:
        ...
        self._previous_results_filename: str = "solscan.db.json"
        self._results_to_hide: List = []
        self._previous_results: List = []
        # From triaged result
        self._previous_results_ids: Set[str] = set()
        # Every solscan object has a list of result from detector
        ...
        # If set to true, solscan will not catch errors during parsing
        ...
        self._compilation_units: List[SolscanCompilationUnit] = []
    ...
    @property
    def compilation_units(self) -> List[SolscanCompilationUnit]:
        return list(self._compilation_units)

    def add_compilation_unit(self, compilation_unit: SolscanCompilationUnit):
    ...
    def parse_ignore_comments(self, file: str) -> None:
        ...
        while True:
            ...
            start_regex = r"^\s*//\s*solscan-disable-start\s*([a-zA-Z0-9_,-]*)"
            end_regex = r"^\s*//\s*solscan-disable-end\s*([a-zA-Z0-9_,-]*)"
                ...
                if ignored:
                    for check in ignored:
                        ...
                        if len(vals) == 0 or vals[-1][1] != float("inf"):
                            ...
                            logger.error(
                                f"Consecutive solscan-disable-starts without solscan-disable-end in {file}#{line_number}"
                            )
                            return
            ...
            if end_match:
                ...
                if ignored:
                    for check in ignored:
                        ...
                        if len(vals) == 0 or vals[-1][1] != float("inf"):
                            logger.error(
                                f"solscan-disable-end without solscan-disable-start in {file}#{line_number}"
                            )
                            return
    ...
    def has_ignore_comment(self, r: Dict) -> bool:
        ...
        for file, lines in mapping_elements_with_lines:
            ...
            if ignore_line_text:
                match = re.findall(
                    r"^\s*//\s*solscan-disable-next-line\s*([a-zA-Z0-9_,-]*)",
        ...
        for path in self._paths_to_filter:
            try:
                ...
                logger.error(
                    ...
                    "\nSolscan supports the Python re format"
