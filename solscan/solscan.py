...
class Solscan(SolscanCore):  # pylint: disable=too-many-instance-attributes
    def __init__(self, target: Union[str, CryticCompile], **kwargs) -> None:
        ...
        self._parsers: List[SolscanCompilationUnitSolc] = []
        try:
            ...
            raise SolscanError(f"Invalid compilation: \n{str(e)}")
        for compilation_unit in crytic_compile.compilation_units.values():
            compilation_unit_solscan = SolscanCompilationUnit(self, compilation_unit)
            self._compilation_units.append(compilation_unit_solscan)
            parser = SolscanCompilationUnitSolc(compilation_unit_solscan)
            self._parsers.append(parser)
            for path, ast in compilation_unit.asts.items():
                parser.parse_top_level_from_loaded_json(ast, path)
                self.add_source_code(path)

            _update_file_scopes(compilation_unit_solscan.scopes.values())
