...
def test_abstract_contract(solc_binary_path) -> None:
    ...
    solscan = Solscan(Path(CONTRACT_DECL_TEST_ROOT, "abstract.sol").as_posix(), solc=solc_path)
    assert not solscan.contracts[0].is_fully_implemented
...
def test_concrete_contract(solc_binary_path) -> None:
    solc_path = solc_binary_path("0.8.0")
    solscan = Solscan(Path(CONTRACT_DECL_TEST_ROOT, "concrete.sol").as_posix(), solc=solc_path)
    assert solscan.contracts[0].is_fully_implemented
...
def test_private_variable(solc_binary_path) -> None:
    ...
    contract_c = solscan.get_contract_from_name("C")[0]
