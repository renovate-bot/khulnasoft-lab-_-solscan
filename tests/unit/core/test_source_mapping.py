...
def test_source_mapping(solc_binary_path):
    ...
    functions = solscan.offset_to_objects(file, 27)
    ...
    assert {(x.start, x.end) for x in solscan.offset_to_definitions(file, 27)} == {(26, 28)}
    # Only one reference for A.f(), in A.test()
    assert {(x.start, x.end) for x in solscan.offset_to_references(file, 27)} == {(92, 93)}
    # Only one implementation for A.f(), in A.test()
    assert {(x.start, x.end) for x in solscan.offset_to_implementations(file, 27)} == {(17, 53)}

    # Check if C.f() is at the offset 203
    functions = solscan.offset_to_objects(file, 203)
    assert len(functions) == 1
    function = functions.pop()
    assert isinstance(function, Function)
    ...
    assert {(x.start, x.end) for x in solscan.offset_to_definitions(file, 203)} == {(202, 204)}
    # Two references for C.f(), in A.test() and C.test2()
    assert {(x.start, x.end) for x in solscan.offset_to_references(file, 203)} == {
        (270, 271),
        (92, 93),
    }
    # Only one implementation for A.f(), in A.test()
    assert {(x.start, x.end) for x in solscan.offset_to_implementations(file, 203)} == {(193, 230)}
    ...
    functions = solscan.offset_to_objects(file, 93)
    print(functions)
    ...
    assert {(x.start, x.end) for x in solscan.offset_to_definitions(file, 93)} == {
        (26, 28),
        (202, 204),
        (138, 140),
    }
    ...
    assert {(x.start, x.end) for x in solscan.offset_to_references(file, 93)} == {
        (92, 93),
        (270, 271),
    }

    # There are three implementations possible (in A, B or C)
    assert {(x.start, x.end) for x in solscan.offset_to_implementations(file, 93)} == {
        (17, 53),
        (193, 230),
        (129, 166),
    }
...
def test_references_user_defined_types_when_casting(solc_binary_path):
    ...
    contracts = solscan.compilation_units[0].contracts
