"""
tests for `solscan.core.declarations.Function`.
tests that `tests/test_function.sol` gets translated into correct
`solscan.core.declarations.Function` objects or its subclasses
...
def test_functions(solc_binary_path):
    ...
    functions = solscan.get_contract_from_name("TestFunction")[0].available_functions_as_dict()
...
def test_function_can_send_eth(solc_binary_path):
    ...
    file = Path(FUNC_DELC_TEST_ROOT, "test_function.sol").as_posix()
    solscan = Solscan(file, solc=solc_path)
    compilation_unit = solscan.compilation_units[0]
...
def test_public_variable(solc_binary_path) -> None:
    ...
    file = Path(FUNC_DELC_TEST_ROOT, "test_function.sol").as_posix()
    solscan = Solscan(file, solc=solc_path)
    contracts = solscan.get_contract_from_name("TestFunction")
