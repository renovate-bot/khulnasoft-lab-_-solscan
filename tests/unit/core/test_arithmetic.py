...
from solscan.utils.arithmetic import unchecked_arithemtic_usage
from solscan.slithir.operations import Binary, Unary, Assignment
...
def test_arithmetic_usage(solc_binary_path) -> None:
    ...
    solscan = Solscan(Path(TEST_DATA_DIR, "test.sol").as_posix(), solc=solc_path)

    assert {
        f.source_mapping.content_hash for f in unchecked_arithemtic_usage(solscan.contracts[0])
...
def test_scope_is_checked(solc_binary_path) -> None:
    solc_path = solc_binary_path("0.8.15")
    solscan = Solscan(Path(TEST_DATA_DIR, "unchecked_scope.sol").as_posix(), solc=solc_path)
    func = solscan.get_contract_from_name("TestScope")[0].get_function_from_full_name(
        "scope(uint256)"
    )
    bin_op_is_checked = {}
