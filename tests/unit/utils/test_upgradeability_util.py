...
from solscan.utils.upgradeability import (
    compare,
    get_proxy_implementation_var,
    get_proxy_implementation_slot,
)

SOLSCAN_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
...
def test_upgrades_compare(solc_binary_path) -> None:
    ...
    sl = Solscan(os.path.join(TEST_DATA_DIR, "TestUpgrades-0.8.2.sol"), solc=solc_path)
...
def test_upgrades_implementation_var(solc_binary_path) -> None:
    ...
    sl = Solscan(os.path.join(TEST_DATA_DIR, "TestUpgrades-0.5.0.sol"), solc=solc_path)
    ...
    # # The util fails with this proxy due to how Solscan parses assembly w/ Solidity versions < 0.6.0 (see issue #1775)
