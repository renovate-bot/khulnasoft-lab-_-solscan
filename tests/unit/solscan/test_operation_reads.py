...
from solscan.slithir.operations import Operation, NewContract
...
def test_operation_reads(solc_binary_path) -> None:
    ...
    solscan = Solscan(Path(TEST_DATA_DIR, "operation_reads.sol").as_posix(), solc=solc_path)

    for op_test in OPERATION_TEST:
        print(op_test)
        available = solscan.get_contract_from_name(op_test.contract_name)
