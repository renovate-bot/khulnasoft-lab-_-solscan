...
from solscan.core.cfg.node import Node, NodeType
from solscan.core.declarations import Function, Contract
...
from solscan.slithir.utils.ssa import is_used_later
from solscan.slithir.variables import (
    Constant,
    ReferenceVariable,
    LocalIRVariable,
    StateIRVariable,
...
def verify_properties_hold(solscan: Solscan) -> None:
    ...
    def verify(solscan: Solscan) -> None:
        for cu in solscan.compilation_units:
    ...
    assert isinstance(solscan, Solscan)
    verify(solscan)
...
def test_multi_write(solscan_from_source) -> None:
    source = """
    ...
    with solscan_from_source(source) as solscan:
        verify_properties_hold(solscan)


def test_single_branch_phi(solscan_from_source) -> None:
    source = """
        pragma solidity ^0.8.11;
        contract Test {
...
def test_basic_phi(solscan_from_source) -> None:
    source = """
    pragma solidity ^0.8.11;
    contract Test {
...
def test_basic_loop_phi(solscan_from_source) -> None:
    source = """
    pragma solidity ^0.8.11;
    contract Test {
...
@pytest.mark.xfail(strict=True, reason="Fails in current solscan version. Fix in #1102.")
def test_phi_propagation_loop(solscan_from_source):
    source = """
     pragma solidity ^0.8.11;
     contract Test {
...
@pytest.mark.xfail(strict=True, reason="Fails in current solscan version. Fix in #1102.")
def test_free_function_properties(solscan_from_source):
    source = """
        pragma solidity ^0.8.11;
...
def test_ssa_inter_transactional(solscan_from_source) -> None:
    source = """
    pragma solidity ^0.8.11;
    contract A {
        uint my_var_A;
...
@pytest.mark.xfail(strict=True, reason="Fails in current solscan version. Fix in #1102.")
def test_ssa_phi_callbacks(solscan_from_source):
    source = """
    pragma solidity ^0.8.11;
    contract A {
        uint my_var_A;
    ...
    """
    with solscan_from_source(source) as solscan:
        c = solscan.get_contract_from_name("A")[0]
        _dump_functions(c)
...
@pytest.mark.xfail(strict=True, reason="Fails in current solscan version. Fix in #1102.")
def test_storage_refers_to(solscan_from_source):
...
def test_initial_version_exists_for_locals(solscan_from_source):
    """
    ...
    """
    with solscan_from_source(src, "0.4.0") as solscan:
        verify_properties_hold(solscan)
        c = solscan.contracts[0]
        f = c.functions[0]
...
def test_initial_version_exists_for_state_variables(solscan_from_source):
    """
    In solidity you can write statements such as
...
@pytest.mark.xfail(strict=True, reason="Fails in current solscan version. Fix in #1102.")
def test_initial_version_exists_for_state_variables_function_assign(solscan_from_source):
    """
    In solidity you can write statements such as
    uint a = a + 1, this test ensures that can be handled for local variables.
    """
    ...
    """
    with solscan_from_source(src) as solscan:
        verify_properties_hold(solscan)
        c = solscan.contracts[0]
        f, ctor = c.functions
...
def test_return_local_before_assign(solscan_from_source):
    src = """
    ...
    """
    with solscan_from_source(src, "0.4.0") as solscan:
        f = solscan.contracts[0].functions[0]
...
def test_shadow_local(solscan_from_source):
    src = """
    contract A {
     // this require solidity 0.5
    ...
    """
    with solscan_from_source(src, "0.5.0") as solscan:
        _dump_functions(solscan.contracts[0])
        f = solscan.contracts[0].functions[0]
...
@pytest.mark.xfail(strict=True, reason="Fails in current solscan version. Fix in #1102.")
def test_multiple_named_args_returns(solscan_from_source):
...
def test_memory_array(solscan_from_source):
    src = """
    contract MemArray {
        struct A {
            uint val1;
...
@pytest.mark.xfail(reason="Tests for wanted state of SSA IR, not current.", strict=True)
def test_storage_array(solscan_from_source):
    src = """
...
@pytest.mark.xfail(strict=True, reason="Fails in current solscan version. Fix in #1102.")
def test_issue_468(solscan_from_source):
    """
    Ensure issue 468 is corrected as per
    https://github.com/khulnasoft-lab/solscan/issues/468#issuecomment-620974151
    ...
    """
    with solscan_from_source(source) as solscan:
        c = solscan.get_contract_from_name("State")[0]
...
@pytest.mark.xfail(strict=True, reason="Fails in current solscan version. Fix in #1102.")
def test_issue_434(solscan_from_source):
    source = """
     contract Contract {
        int public a;
        function f() public {
    ...
    """
    with solscan_from_source(source) as solscan:
        c = solscan.get_contract_from_name("Contract")[0]
...
@pytest.mark.xfail(strict=True, reason="Fails in current solscan version. Fix in #1102.")
def test_issue_473(solscan_from_source):
    source = """
    contract Contract {
...
def test_issue_1748(solscan_from_source):
    source = """
    contract Contract {
        uint[] arr;
...
def test_issue_1776(solscan_from_source):
    source = """
    contract Contract {
...
def test_issue_1846_ternary_in_if(solscan_from_source):
    source = """
    contract Contract {
...
def test_issue_1846_ternary_in_ternary(solscan_from_source):
    source = """
        contract Contract {
            function foo(uint x) public returns (uint y) {
                y = x > 0 ? x > 1 ? 2 : 3 : 4;
