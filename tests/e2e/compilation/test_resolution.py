...
def test_node_modules() -> None:
    ...
    solscan = Solscan(Path(TEST_DATA_DIR, "test_node_modules").as_posix())
    _run_all_detectors(solscan)
...
def test_cycle(solc_binary_path) -> None:
    solc_path = solc_binary_path("0.8.0")
    solscan = Solscan(Path(TEST_DATA_DIR, "test_cyclic_import", "a.sol").as_posix(), solc=solc_path)
    _run_all_detectors(solscan)
