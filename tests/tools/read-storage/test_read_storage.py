...
from solscan.tools.read_storage import SolscanReadStorage, RpcInfo
...
def test_read_storage(test_contract, storage_file, web3, ganache, solc_binary_path) -> None:
    ...
    sl = Solscan(Path(TEST_DATA_DIR, f"{test_contract}.sol").as_posix(), solc=solc_path)
    contracts = sl.contracts

    rpc_info: RpcInfo = RpcInfo(ganache.provider)
    srs = SolscanReadStorage(contracts, 100, rpc_info)
